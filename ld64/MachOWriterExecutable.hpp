...
#ifdef DARWIN
#define uuid_generate_random(uuid) (uuid_generate_random(uuid))
#else
#define uuid_generate_random(uuid) uuid_generate_random_impl(uuid)
#define CC_MD5_DIGEST_LENGTH (32)
static void uuid_generate_random_impl (void *uuid) {
	int fd = open("/dev/urandom", O_RDONLY, 0);
	if (fd == -1) {
		printf("can't open: /dev/urandom to fill in uuid load command (using bytes of zero)\n");
		memset(uuid, '\0', CC_MD5_DIGEST_LENGTH);
	} else {
		ssize_t n = read(fd, uuid, CC_MD5_DIGEST_LENGTH);
		if (n != CC_MD5_DIGEST_LENGTH) {
			printf("can't read bytes from: /dev/urandom to fill in uuid load command (using bytes of zero)\n");
			memset(uuid, '\0', CC_MD5_DIGEST_LENGTH);
		}
		close(fd);
	}
...
#else
			// just generate random UUID for other platforms
			::uuid_generate_random(digest);
			fUUIDAtom->setContent(digest);
			uint32_t uuidOffset = ((SectionInfo*)fUUIDAtom->getSection())->fFileOffset + fUUIDAtom->getSectionOffset();
			if ( streaming ) {
...
#else
	return this->alignedSize(16 + 42 * 4); 
#endif
}
