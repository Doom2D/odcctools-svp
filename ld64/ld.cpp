...
#ifndef DARWIN
static uint64_t mach_absolute_time () {
	return 0;
}
#endif
...
void Linker::printTime(const char* msg, uint64_t partTime, uint64_t totalTime)
{
	...
	if ( sUnitsPerSecond == 0 ) {
		sUnitsPerSecond = 1000; // TODO fix this
#ifdef DARWIN
		...
		switch ( it->type ) {
			...
			case N_SO:
				if ( (it->string != NULL) && (strlen(it->string) > 0) ) {
					...
					soRanges.push_back(std::make_pair<ObjectFile::Atom*,ObjectFile::Atom*>(std::move(atomWithLowestOrdinal), std::move(atomWithHighestOrdinal)));
				}
				// fall through
			default:
				if ( curRangeIndex != -1 ) {
